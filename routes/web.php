...
use App\Http\Controllers\Backend\ApprovalInboxController;
...
use App\Http\Controllers\Backend\BarcodeLabelController;
use App\Http\Controllers\Backend\BinTransferController;
...
use App\Http\Controllers\Backend\ComparisonStatementController;
...
use App\Http\Controllers\Backend\FeatureToggleController;
...
use App\Http\Controllers\Backend\LetterOfCreditController;
use App\Http\Controllers\Backend\MonthEndSnapshotController;
use App\Http\Controllers\Backend\OrderApprovalController;
use App\Http\Controllers\Backend\OutletController;
use App\Http\Controllers\Backend\PayablesReceivablesReportController;
use App\Http\Controllers\Backend\PaymentSettingController;
use App\Http\Controllers\Backend\PaymentTransactionController;
use App\Http\Controllers\Backend\PayPalPaymentController;
...
use App\Http\Controllers\Frontend\InvoicePaymentController;
...
Route::middleware('auth')->group(function () {
    ...
    Route::group(['middleware' => ['auth', 'check.permission'], 'prefix' => 'admin', 'as' => 'admin.'], function () {
        ...
        Route::get('rfqs/{rfq}/cs', [ComparisonStatementController::class, 'create'])->name('rfqs.cs.create');
        Route::post('rfqs/{rfq}/cs', [ComparisonStatementController::class, 'store'])->name('rfqs.cs.store');
        Route::post('rfqs/cs/{cs}/approve', [ComparisonStatementController::class, 'approve'])->name('rfqs.cs.approve');
        Route::post('rfqs/cs/{cs}/reject', [ComparisonStatementController::class, 'reject'])->name('rfqs.cs.reject');
        Route::get('rfqs/{rfq}/cs/{cs}/pdf', [ComparisonStatementController::class, 'downloadPdf'])->name('rfqs.cs.pdf');
        Route::get('rfqs/{rfq}/cs/{cs}/pdf/view', [ComparisonStatementController::class, 'streamPdf'])->name('rfqs.cs.pdf.view');
        ...
        Route::post('proforma-invoices/store', [ProformaInvoiceController::class, 'store'])->name('proforma-invoices.store');

        Route::controller(LetterOfCreditController::class)->group(function () {
        ...
        Route::controller(LandedCostController::class)->group(function () {
        ...
        Route::controller(VendorReturnController::class)->group(function () {
        ...
        Route::controller(ReportController::class)->group(function () {
            ...
            Route::get('reports/audit/pdf/async', 'auditReportPdfAsync')->name('reports.audit.pdf.async');
        ...
        Route::controller(FeatureToggleController::class)->group(function () {
        ...
        Route::controller(PaymentSettingController::class)->prefix('payment-settings')->name('payment-settings.')->group(function () {
            Route::get('/', 'index')->name('index');
        ...
        Route::controller(CodManagementController::class)->group(function () {
        ...
        Route::get('bin-transfers/create', [BinTransferController::class, 'create'])->name('bin-transfers.create');
        Route::post('bin-transfers', [BinTransferController::class, 'store'])->name('bin-transfers.store');
        Route::get('bin-transfers/get-bin-products', [BinTransferController::class, 'getBinProducts'])->name('bin-transfers.get-bin-products');
        ...
        /** Enterprise Product Barcode Labels Engine */
        Route::controller(BarcodeLabelController::class)->prefix('barcode-labels')->name('barcode-labels.')->group(function () {
            Route::get('/', 'index')->name('index');
            Route::get('/search-products', 'searchProducts')->name('search-products');
            Route::post('/generate-barcode', 'generateBarcode')->name('generate-barcode');
            Route::post('/generate-all-missing', 'generateAllMissing')->name('generate-all-missing');
        ...
        Route::resource('fiscal-years', FiscalYearController::class)->only(['index', 'store', 'update']);
        Route::post('fiscal-years/{fiscalYear}/toggle-close', [FiscalYearController::class, 'toggleClose'])->name('fiscal-years.toggle-close');
        ...
        Route::resource('bank-accounts', BankAccountController::class)->except(['create', 'show']);
        Route::get('bank-reconciliation', [BankReconciliationController::class, 'index'])->name('bank-reconciliation.index');
        Route::post('bank-reconciliation', [BankReconciliationController::class, 'reconcile'])->name('bank-reconciliation.reconcile');

        /** Petty Cash Register Routes */
        Route::resource('petty-cash', PettyCashController::class)->only(['index', 'store']);

        /** Fund Transfers (Contra) Routes */
        Route::post('fund-transfers/{id}/approve', [FundTransferController::class, 'approve'])->name('fund-transfers.approve');
        Route::post('fund-transfers/{id}/reject', [FundTransferController::class, 'reject'])->name('fund-transfers.reject');
        Route::resource('fund-transfers', FundTransferController::class)->only(['index', 'store']);
        ...
        Route::get('journal-vouchers', [JournalVoucherController::class, 'index'])->name('journal-vouchers.index');
        Route::get('journal-vouchers/create', [JournalVoucherController::class, 'create'])->name('journal-vouchers.create');
        Route::post('journal-vouchers', [JournalVoucherController::class, 'store'])->name('journal-vouchers.store');
        ...
        Route::post('approvals/{id}/approve', [ApprovalInboxController::class, 'approve'])->name('approvals.approve');
        Route::post('approvals/{id}/reject', [ApprovalInboxController::class, 'reject'])->name('approvals.reject');

        /** Phase 2 Step 4: Vendor Bills, Payments, Ledger, Returns & Reports */
        Route::post('vendor-bills/{id}/approve', [VendorBillController::class, 'approve'])->name('vendor-bills.approve');
        Route::post('vendor-bills/{id}/reject', [VendorBillController::class, 'reject'])->name('vendor-bills.reject');
        ...
        Route::controller(SalesInvoiceController::class)->group(function () {
            ...
            Route::post('sales-invoices/{salesInvoice}/send-payment-link', [InvoicePaymentController::class, 'sendEmail'])->name('sales-invoices.send-payment-link');
        });
        ...
        Route::controller(PayablesReceivablesReportController::class)->group(function () {
