                ...
                <div class="pp-card-img-wrap d-flex align-items-center justify-content-center position-relative">
                    @php
                        $imgSrc = $product->thumb_image
                            ? (strpos($product->thumb_image, 'http') === 0
                                ? $product->thumb_image
                                : asset('storage/' . $product->thumb_image))
                            : asset('uploads/no-image.svg');
                    @endphp
                    <img alt="{{ $product->name }}" src="{{ $imgSrc }}"
                        onerror="this.onerror=null; this.src='{{ asset('uploads/no-image.svg') }}';" class="img-fluid"
                    ...
                    <div class="pp-card-badges" style="top: 6px; left: 6px; gap: 3px;">
                        ...
                        @if ($product->product_type)
                            <span
                                class="badge {{ $product->product_type == 'new_arrival' ? 'badge-success' : ($product->product_type == 'upcoming' ? 'badge-warning' : 'badge-primary') }} pp-badge-type shadow-sm"
                                style="{{ $product->product_type == 'upcoming' ? 'color: #000;' : '' }}">
                    ...
                    <div class="pp-card-actions" style="top: 6px; right: 6px; gap: 4px;">
                        @if (Auth::user()->can('Manage Products'))
                            <label class="custom-switch m-0 pp-status-switch">
                                <input type="checkbox" name="custom-switch-checkbox" data-id="{{ $product->id }}"
                                    class="custom-switch-input change-status" {{ $product->status ? 'checked' : '' }}>
                                <span class="custom-switch-indicator shadow-sm"></span>
                            </label>
                        @else
                            <span
                                class="badge {{ $product->status ? 'badge-success' : 'badge-danger' }} shadow-sm px-2 py-1"
                                style="border-radius: 6px; font-size: 9px;">{{ $product->status ? 'Active' : 'Inactive' }}</span>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="pp-price-box">
                            ...
                            <div class="pp-variant-scroll">
                                @foreach ($product->variants as $v)
                                        ...
                                        if ($variantLabel === '') {
                                            $variantLabel = trim(
                                                implode(
                                                    ' ',
                                                    array_filter([
                                                        is_object($v->getRelation('color') ?? null)
                                                            ? optional($v->getRelation('color'))->name
                                                            : null,
                                                        is_object($v->getRelation('size') ?? null)
                                                            ? optional($v->getRelation('size'))->name
                                                            : null,
                                                        $v->color ?? null,
                                                        $v->size ?? null,
                                                    ]),
                                    ...
                                    <div class="d-flex justify-content-between align-items-center py-1 {{ !$loop->last ? 'border-bottom' : '' }}"
                                        style="border-color: var(--pp-border) !important;">
                                        <span
                                            style="font-size: 9px; font-weight: 600; color: var(--pp-muted);">{{ $variantLabel }}</span>
                        ...
                        @else
                            @if ($userCanManage)
                                <div class="d-flex justify-content-between align-items-center py-1 border-bottom"
                                    ...
                                    <span class="pp-price-value"
                                        style="color: #6b7280; font-size: 11px;">{{ formatConverted($product->purchase_price) }}</span>
                                </div>
                                <div class="d-flex justify-content-between align-items-center py-1 border-bottom"
                                    style="border-color: var(--pp-border) !important;">
                                    <span class="pp-price-label">Wholesale</span>
                                    <span class="pp-price-value"
                                        style="color: #16a34a; font-size: 11px;">{{ formatConverted($product->outlet_price) }}</span>
                                </div>
                                <div class="d-flex justify-content-between align-items-center pt-1">
                                    <span class="pp-price-label">Retail</span>
                                    <span class="pp-price-value"
                                        style="font-size: 11px;">{{ formatConverted($product->price) }}</span>
                                </div>
                            @elseif($isOutletUser)
                        ...
                        <div class="mt-2 d-flex" style="gap: 6px;">
                            @can('Manage Order Place')
                                <button type="button" class="btn btn-sm pp-btn-card pp-btn-amber flex-fill add-to-basket"
                                    data-id="{{ $product->id }}" data-has-variants="{{ $hasVariants ? 1 : 0 }}"
                                    title="Procurement Basket"
                                    style="padding: 7px 0; font-size: 13px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-shopping-basket"></i>
                                </button>
                            @endcan
                            @if (Auth::user()->can('Manage Sales Quotations') ||
                                    Auth::user()->can('Create Product Requests') ||
                                    Auth::user()->hasRole('Admin'))
                                <button type="button"
                                    class="btn btn-sm pp-btn-card pp-btn-outline flex-fill add-to-request-basket"
                                    data-id="{{ $product->id }}" data-has-variants="{{ $hasVariants ? 1 : 0 }}"
...
<div class="row mt-3 mb-4">
    <div class="col-12">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center pp-pagination-wrapper"
            style="gap: 8px;">
            <span class="pp-count-badge">
                <i class="fas fa-list-ul mr-1" style="opacity: 0.5; font-size: 9px;"></i>
                Showing
                <strong>{{ $products->firstItem() ?? 0 }}</strong>–<strong>{{ $products->lastItem() ?? 0 }}</strong>
