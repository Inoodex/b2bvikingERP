                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.warehouse-zones.update', $warehouseZone->id) }}" method="POST">
                            ...
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="outlet_id" name="outlet_id" class="form-control select2 @error('outlet_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($outlets as $outlet)
                                            <option value="{{ $outlet->id }}" {{ (old('outlet_id', $warehouseZone->outlet_id) == $outlet->id) ? 'selected' : '' }}>
                                                {{ $outlet->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" 
                                        value="{{ old('name', $warehouseZone->name) }}" required>
                                    @error('name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="type" name="type" class="form-control @error('type') is-invalid @enderror" required>
                                        <option value="active" {{ old('type', $warehouseZone->type) == 'active' ? 'selected' : '' }}>Active Storage</option>
                                        <option value="quarantine" {{ old('type', $warehouseZone->type) == 'quarantine' ? 'selected' : '' }}>Quarantine (QC Hold)</option>
                                        <option value="scrap" {{ old('type', $warehouseZone->type) == 'scrap' ? 'selected' : '' }}>Scrap / Damaged</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="status" class="form-control" name="status">
                                        <option value="1" {{ old('status', $warehouseZone->status) == 1 ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ old('status', $warehouseZone->status) == 0 ? 'selected' : '' }}>Inactive</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="text-right mt-3">
                                ...
                                <button type="submit" class="btn btn-primary px-4">Update Zone</button>
                            </div>
                        </form>
                    </div>
                </div>
