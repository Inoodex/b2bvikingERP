...
$(document).ready(function() {
    $('#btn-generate-pdf').on('click', function(e) {
        ...
        if (!generateUrl) {
            console.error('No generateUrl found on #btn-generate-pdf');
            return;
        }

        // Capture current form filter values if a form exists
        var $form = $('form#filter-form, form#vendor-payment-filter-form, form[action*="reports"], form[action*="accounts"], form[action*="vendor-ledger"]').first();
        var formData = '';
        ...
        // If DataTable exists, include its search if present
        if (typeof LaravelDataTables !== 'undefined') {
            for (var key in LaravelDataTables) {
                if (LaravelDataTables[key] && typeof LaravelDataTables[key].search === 'function') {
                    var dtSearch = LaravelDataTables[key].search();
                    if (dtSearch && formData.indexOf('search=') === -1) {
                        formData += (formData ? '&' : '') + 'search=' + encodeURIComponent(dtSearch);
                    }
                    break;
                }
            }
        ...
        $.ajax({
            ...
            success: function(response) {
                var checkUrl = "{{ route('admin.reports.payables-receivables.check-status') }}";
                var pollAttempts = 0;
